...
function ScoreEventLog(events, scores, traitors, detectives, maniacs)
   for k, s in pairs(scores) do
      ...
      scores[k].was_maniac = table.HasValue(maniacs, k)
   end
...
function ScoreTeamBonus(scores, wintype)
   local alive = {traitors = 0, maniacs = 0, innos = 0}
   local dead = {traitors = 0, maniacs = 0, innos = 0}
   ...
   for k, sc in pairs(scores) do
      ...
      elseif sc.was_maniac then
         state.maniacs = state.maniacs + 1
      else
   ...
   bonus.maniacs = (alive.maniacs * 1) + math.ceil(dead.traitors * 0.7) + math.ceil(dead.innos * 0.5)
